...
#(
  ...
  for (genvar i = 0; i < CVA6Cfg.ICACHE_SET_ASSOC; i++) begin : gen_tag_cmpsel
    ...
    if (CVA6Cfg.FETCH_USER_EN) begin : gen_user_sel
      assign cl_user[i] = cl_ruser[i][{cl_offset_q, 3'b0}+:CVA6Cfg.FETCH_USER_WIDTH];
    end else begin : gen_no_user_sel
      assign cl_user[i] = '0;
    end
  end
  ...
  logic [CVA6Cfg.FETCH_USER_WIDTH-1:0] mem_rtrn_user;

  if (CVA6Cfg.FETCH_USER_EN) begin : gen_rtrn_user_sel
    assign mem_rtrn_user = mem_rtrn_i.user[{cl_offset_q, 3'b0}+:CVA6Cfg.FETCH_USER_WIDTH];
  end else begin : gen_no_rtrn_user_sel
    assign mem_rtrn_user = '0;
  end
  ...
  assign icache_phys_addr = {
    areq_i.fetch_paddr[CVA6Cfg.PLEN-1:CVA6Cfg.ICACHE_INDEX_WIDTH],
    vaddr_q[CVA6Cfg.ICACHE_INDEX_WIDTH-1:0]
  };
  ...
  for (genvar i = 0; i < CVA6Cfg.ICACHE_SET_ASSOC; i++) begin : gen_sram
    always_comb begin
      ...
      if (icache_spm_ways_i[i]) begin
        ...
        ram_be[i] = {
          {((CVA6Cfg.ICACHE_TAG_WIDTH + 1 + 7) / 8) {vld_we}},
          {(CVA6Cfg.ICACHE_LINE_WIDTH / 8) {cl_we}}
        };
