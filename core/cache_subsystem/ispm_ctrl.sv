...
#(
  ...
  logic [$clog2(LINE_WIDTH/8)-$clog2(riscv::XLEN/8)-1:0]
      lsu_cl_offset, lsu_cl_offset_d, lsu_cl_offset_q;
  ...
  assign lsu_cl_offset = spm_rw_req_port_i.address_index[$clog2(
      LINE_WIDTH/8
  )-1:$clog2(
      riscv::XLEN/8
  )];

  assign lsu_addr = {NR_WAYS{spm_rw_req_port_i.address_index[$clog2(
      LINE_WIDTH/8
  )+:(IDX_WIDTH-$clog2(
      LINE_WIDTH/8
  ))]}};
  ...
  assign read_addr_req = icache_req_port_i.vaddr[$clog2(
      LINE_WIDTH/8
  )+:(IDX_WIDTH-$clog2(
      LINE_WIDTH/8
  ))];
  ...
  assign read_addr_phys = icache_phys_addr_i[$clog2(
      LINE_WIDTH/8
  )+:(IDX_WIDTH-$clog2(
      LINE_WIDTH/8
  ))];
  ...
  always_comb begin
    ...
    if (lsu_wait_stage_q) lsu_wait_stage_d = lsu_wait_stage_q - 32'b1;
