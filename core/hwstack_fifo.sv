...
module hwstack_fifo #(
    ...
    parameter int unsigned AddrDepth = (CVA6Cfg.HwstackFifoDepth > 1) ? $clog2(
        CVA6Cfg.HwstackFifoDepth
    ) : 1
) (
    input  logic             clk_i,
    input  logic             rst_ni,
  ...
  always_comb begin : read_write_comb
    ...
    if (push_i && ~full_o) begin
      ...
      gate_clock = 1'b0;
      if (write_pointer_q == FifoDepth[AddrDepth-1:0] - 1) write_pointer_n = '0;
    ...
    if (pop_i && ~empty_o) begin
      if (read_pointer_n == FifoDepth[AddrDepth-1:0] - 1) read_pointer_n = '0;
