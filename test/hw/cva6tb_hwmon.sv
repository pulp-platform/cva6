package cva6tb_hwmon_pkg;
  ...
  //   ABORT     0x14  wdata = task_id  → job killed at its deadline; emit a
  //                                       JOB_ABORT event and close the window.
  //                                       Without this an overrunning job writes
  //                                       no STOP and vanishes from the log.
  //   MARK      0x18  wdata = value    → arbitrary software probe point
  ...
  localparam logic [IntAddrWidth-1:0] CVA6TB_HWMON_ABORT_OFFSET     = 5'h14;
  localparam logic [IntAddrWidth-1:0] CVA6TB_HWMON_MARK_OFFSET      = 5'h18;
...
#(
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      case (req_addr)
        ...
        // --- Job killed at its deadline: close the window, same as STOP. The
        //     event row is emitted by the logging block below. ---
        CVA6TB_HWMON_ABORT_OFFSET: begin
          if (active_q) begin
            exec_time_q[current_task_q] <=
                exec_time_q[current_task_q] + (cycle_q - window_start_q);
            active_q <= 1'b0;
  ...
  // ---------------------------------------------------------------------------
  // ---------------------------------------------------------------------------
  // Event stream — events.csv
  //
  // One row per event, in cycles from the module's own free-running counter, so
  // there is a single time base rather than the previous mix of $realtime in ns
  // (interrupt log), cycles (per-job log) and mtime ticks (deadlines).
  //
  // Fixed schema; which columns are meaningful depends on `event`:
  //
  //   JOB_RELEASE  task, extra = absolute deadline (mtime ticks)
  //   JOB_START    task, extra = preempted task id, or 0 if none
  //   JOB_STOP     task, exec_cycles .. dtlb_misses  (the full per-job payload)
  //   JOB_ABORT    task, exec_cycles, response_time  (partial: job was killed)
  //   MARK         extra = the value software wrote
  //   RUN_READY    first release of a task with a committed period: the
  //                scheduler is up. Replaces a hardcoded start delay.
  //   RUN_DONE     the measured task reached +NumJobs completed jobs.
  //
  // JOB_ABORT is why this exists: a job killed at its deadline writes no STOP,
  // so it produced no row at all and a missed deadline read as missing data.
  // JOB_RELEASE is always emitted, so a release with no matching STOP is
  // visible as an overrun rather than as an absent sample.
  // ---------------------------------------------------------------------------
  localparam string FileName    = "hwmon_log.csv";   // transitional, see below
  localparam string EventsFile  = "events.csv";
  integer log_file;
  integer ev_file;

  // Run control. Read here rather than plumbed through the SoC: the testbench
  // already reaches into this module hierarchically.
  int unsigned num_jobs;       // 0 = run until MaxCycles
  int unsigned measured_task;
  ...
  function automatic void ev(string name, int unsigned task_id,
                             ...
                             longint unsigned extra);
    $fwrite(ev_file, "%0d,%s,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d\n",
            cycle_q, name, task_id, c0, c1, c2, c3, c4, c5, c6, c7, extra);
    $fflush(ev_file);
  endfunction

  function automatic void setup();
    // hwmon_log.csv is kept for one transition so the new stream can be checked
    // against it row for row; it is removed once the harness parses events.csv.
    log_file = $fopen(FileName, "w");
    if (log_file == 0) $fatal(1, "could not open %s for writing", FileName);
    ...
    ev_file = $fopen(EventsFile, "w");
    if (ev_file == 0) $fatal(1, "could not open %s for writing", EventsFile);
    $fwrite(ev_file, "cycle,event,task,exec_cycles,response_time,sched_jitter,deadline_miss,preemptions,icache_misses,dcache_misses,itlb_misses,dtlb_misses,extra\n");
    $fflush(ev_file);
  ...
  function automatic void cleanup();
    $display("====== Run summary ======");
    $display("measured task    : %0d", measured_task);
    $display("jobs completed   : %0d%s", jobs_done,
  ...
  always @(posedge clk_i) begin
    if (req_write) begin
      case (req_addr)
        CVA6TB_HWMON_RELEASE_OFFSET: begin
          // hwmon_task_init() writes RELEASE twice around the period staging, so
          // only a release with a committed period is a real job release.
          if (period_q[req_task_id] != 0) begin
            ...
            ev("JOB_RELEASE", req_task_id, 0,0,0,0,0,0,0,0,
               time_i + period_q[req_task_id]);
          end
          mon_log($sformatf("RELEASE task %0d @time=%0d, deadline=%0d",
                   req_task_id, time_i, time_i + period_q[req_task_id]));
        end
        CVA6TB_HWMON_START_OFFSET: begin
          ev("JOB_START", req_task_id, 0,0,0,0,0,0,0,0,
             (active_q && current_task_q != req_task_id) ? current_task_q : 0);
        ...
        CVA6TB_HWMON_ABORT_OFFSET: begin
          if (active_q)
            mon_log($sformatf("ABORT task %0d @time=%0d (deadline overrun)",
                     current_task_q, time_i));
        end
        CVA6TB_HWMON_MARK_OFFSET:
          ev("MARK", current_task_q, 0,0,0,0,0,0,0,0, req_wdata32);
        default: ;
      endcase
    end

    if (req_write && req_addr == CVA6TB_HWMON_STOP_OFFSET && active_q) begin
      ev("JOB_STOP", current_task_q,
         log_exec_cycles, log_response_time, log_sched_jitter,
         64'(log_deadline_miss), preempt_count_q[current_task_q],
         icache_count_q[current_task_q], dcache_count_q[current_task_q],
         itlb_count_q[current_task_q], dtlb_count_q[current_task_q]);
      ...
      if (current_task_q == measured_task) begin
        ...
        if (num_jobs != 0 && jobs_done >= num_jobs && !run_done) begin
          run_done = 1'b1;
          ev("RUN_DONE", current_task_q, 0,0,0,0,0,0,0,0, jobs_done);
        end
      end
    end
