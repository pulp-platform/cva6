...
#(
  ...
  // Active cycles : miss pulses accumulated for current_task_q.
  //
  // Misses on the START and STOP write cycles are excluded (scheduler/ecall
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      // Accumulate miss pulses for the task currently on CPU.
  ...
  //   icache_misses  : I-cache miss pulses attributed to this task
  //   dcache_misses  : D-cache miss pulses
  //   itlb_misses    : ITLB miss pulses
  //   dtlb_misses    : DTLB miss pulses
  // ---------------------------------------------------------------------------
