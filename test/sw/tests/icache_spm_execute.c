...
//
// Configure part of the I-cache as SPM and run code from it.
...
uint64_t __attribute__((noinline)) __attribute__((aligned(8))) test_func(uint64_t a, uint64_t b)
{
    uint64_t tmp = 1;
...
int main(void) {
    ...
    src = (volatile uint64_t *) (test_func);
    dst = (volatile uint64_t *) ICACHE_SPM_ADDR;

    // Copy the payload function (128 bytes) to the I-cache SPM
    ...
    uint64_t (*f)(uint64_t, uint64_t) = (uint64_t (*)(uint64_t, uint64_t)) ICACHE_SPM_ADDR;

    dst--;
    res_spm = f(*src, *dst);
    res_cache = test_func(*src, *dst);
