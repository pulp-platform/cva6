...
run_short_test () {
    ...
    echo "Running test \"${t}\""
    ${simbin} +binary="${hexfile}" +RetCodeSccess=1 +MaxCycles=100000 > ${logfile}
}
...
run_long_test () {
    ...
    logfile="${logdir}/${t}.log"
    echo "Running test \"${t}\""
    ${simbin} +binary="${hexfile}" +RetCodeSccess=1 +MaxCycles=2000000 > ${logfile}
}

run_short_tests () {
    mkdir -p ${logdir}
    export -f run_short_test
    printf '%s\n' "$@" | xargs -n1 -P 8 bash -c 'run_short_test $1' _
}

run_long_tests () {
    mkdir -p ${logdir}
    export -f run_long_test
    printf '%s\n' "$@" | xargs -n1 -P 8 bash -c 'run_long_test $1' _
}
